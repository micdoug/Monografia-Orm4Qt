...
Em seu trabalho intitulado "Um framework de mapeamento objeto-relacional com um exemplo em C++", \textit{Zhang Xiaobing} apresenta um modelo para desenvolvimento de uma biblioteca ORM que pode ser utilizado na maioria das linguagens orientadas a objeto. Em seu modelo, ele define padrões a serem utilizados para resolver problemas como mapeamento de classes simples, herança, composição, associação, e ainda define mecanismos de otimização como criação de um cache de objetos resgatados do banco de dados \cite{xiaobingZhang}.
...
\begin{description}
\item[Camada de objetos (\textit{\textbf{Object Layer}})]
...
\item[Camada de persistência (\textit{\textbf{Storage Layer}})]
